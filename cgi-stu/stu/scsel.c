...
/*	fprintf(cgiOut, "<head><meta charset=\"utf-8\"/><title>查询结果</title>\
	...
	if (sno[0] == '*')
	{
		sprintf(sql, "select score.sno,score.cno,sname,cname,dname,score from student,score,course where student.sno=score.sno and course.cno=score.cno and student.flag=0");
	}
	else
	{
		sprintf(sql, "select score.sno,score.cno,sname,cname,dname,score from student,score,course where  student.sno=score.sno and course.cno=score.cno and score.sno = '%s' and student.flag=0", sno);
	}
